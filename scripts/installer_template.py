...
$s.WorkingDirectory = "{target_path.parent}"
...
def perform_installation(target_dir: Path, create_desktop: bool, create_start_menu: bool, progress_callback=None) -> tuple[bool, str]:
    ...
    icon_file = target_dir / "resources" / "icon.ico"
    if not icon_file.exists():
        icon_file = target_dir / "resources" / "icon_32.png"
...
def run_gui_installer():
    ...
    style.configure("Primary.TButton", font=("Microsoft YaHei", 9, "bold"), background="#2563eb", foreground="#ffffff")
    ...
    path_var = tk.StringVar(value=str(DEFAULT_INSTALL_DIR))
    ...
    progress_bar.pack(fill=tk.X, pady=(6, 4))
    ...
    btn_frame = tk.Frame(root, padx=20, pady=12, bg="#f8fafc", bd=1, relief=tk.RIDGE)
    btn_frame.pack(fill=tk.X, side=tk.BOTTOM)

    btn_cancel = ttk.Button(btn_frame, text="取消", command=root.destroy)
    ...
    def do_install():
        ...
        def _worker():
            ...
            if ok:
                btn_install.config(text="完成并退出", state="normal", command=root.destroy)
                if var_launch.get():
    ...
    btn_install = ttk.Button(btn_frame, text="立即安装 (秒级部署)", style="Primary.TButton", command=do_install)
