...
### 🖥️ 2. 独立桌面可视化管理客户端 (Native Edge WebView2 GUI v3.1.0)
- **全新微前端现代架构**：基于 Windows 原生 **Edge WebView2** 渲染引擎与现代 Fluent / Tailwind 本地设计系统，实现 0.15s 秒级冷启动与 60~120 FPS 显卡硬件直推！
- **重塑为 7 大逻辑严密的现代化分类导航**：彻底解决功能割裂与混乱，层级分明直观：
  - 📊 **仪表盘总览**：当前生效账号卡片（双额度进度条/Pro计划徽章/重置倒计时）、系统服务启停、开机自启开关、一键磁盘深度瘦身。
  - 👥 **账号池与切换**：指标卡条（总数/可用数/紧张数/当前生效）、账号工具栏、多账号卡片流、一键切号与添加弹窗。
  - ⚡ **性能与代理**：GPU 显卡直推、防休眠降频、4GB V8 堆内存、反重力专属独立代理（彻底替代 Proxifier）。
  - 🎨 **界面与外观**：多语言切换（简中/繁中/英文）、顶栏额度胶囊、移除推广按钮、紧凑代码视图。
  - 🛡️ **自愈与通知**：任务异常重试（恢复工作自动复位计数）、额度耗尽熔断告警、Telegram/飞书/企微推送通道配置与实时测试。
  - 📝 **系统提示词**：实时查看/修改/备份全局 `AGENTS.md`，内置 5 大预设角色模板一键套用。
  - 📜 **运行日志**：常驻心跳与长跑任务推送日志即时滚动预览与清空。
